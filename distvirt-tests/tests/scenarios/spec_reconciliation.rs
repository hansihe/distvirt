...
async fn test_image_change_restarts_running_pod() {
    ...
    let old_pod_id = cluster
        ...
        .expect("workload 'echo' not found")
        .pod_id
        .clone()
...
async fn test_add_workload_to_existing_namespace() {
    ...
    cluster.assert_workload_running("ns", "echo-a").await;
    cluster.assert_workload_running("ns", "echo-b").await;

    // The old "echo" workload should be removed.
    let status = cluster.namespace_status("ns").await;
    assert!(
        !status.workloads.contains_key(&WorkloadName("echo".to_string())),
        "old 'echo' workload should be removed after spec update"
    );
}
...
async fn test_remove_workload_from_namespace() {
    ...
    let status = cluster.namespace_status("ns").await;
    assert!(
        !status.workloads.contains_key(&WorkloadName("echo-a".to_string())),
        "echo-a should be removed"
    );
    assert!(
        !status.workloads.contains_key(&WorkloadName("echo-b".to_string())),
...
async fn test_image_change_on_suspended_workload() {
    ...
    cluster
        ...
        .await;
    cluster.converge().await;
    cluster.assert_workload_dormant("ns-img", "web").await;

    // Activate -> Running -> deactivate -> suspend.
    cluster.send_activation_traffic("ns-img", "web-svc").await;
    cluster.assert_workload_running("ns-img", "web").await;

    cluster.deactivate_service("ns-img", "web-svc", &w1).await;
    cluster.advance_past_idle_timeout("ns-img", "web-svc").await;
    cluster.wait_workload_suspended("ns-img", "web").await;
    cluster.assert_workload_suspended("ns-img", "web").await;
