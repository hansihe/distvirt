...
async fn test_multi_worker_reschedule_on_disconnect_e2e() {
    ...
    let hosting_worker = cluster
        .workload_state("ns", "echo")
        .worker_id()
        .expect("workload should have a worker_id when Running")
        .clone();
    ...
    cluster.assert_workload_running("ns", "echo");
    let new_worker = cluster
        .workload_state("ns", "echo")
        .worker_id()
        .expect("workload should have a worker_id after reschedule")
        .clone();
    assert_eq!(
