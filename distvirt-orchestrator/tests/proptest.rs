...
fn check_namespace_invariants(ns: &NamespaceStateMachine, output: &NamespaceOutput) {
    ...
    // number of services with wants_backend() + has_active_flows.
    for (wl_id, wl) in &ns.workloads {
        ...
        assert_eq!(
            ...
            "Workload {:?} current_demand={} but expected {} (services={}, active_flows={})",
            wl_id, wl.current_demand, expected_demand, service_demand, active_flows
        );
    }
