...
use super::helpers::{convert_buffer, convert_ports, parse_duration_ms, resolve_resources};
...
pub fn spec_to_namespace_spec(parsed: &super::parse::ParsedSpec) -> Result<(Option<String>, NamespaceSpec), SpecError> {
    ...
    // Validate explicit IP format (basic format check only — the orchestrator
    // handles zone validation, collision detection, and allocation).
    validate_explicit_ips(spec, &mut errs);
    ...
    // IPs are NOT allocated here — the orchestrator handles IP allocation.
    // Explicit IPs are passed through; auto-assigned resources get empty IP strings.
    build_namespace_spec(spec, namespace_id, subnet_str)
}
...
/// Validate explicit IP addresses (format check only).
///
/// The orchestrator handles zone validation, collision detection, and allocation.
/// The client just checks that specified IPs are valid IPv4 and catches duplicates.
fn validate_explicit_ips(spec: &SpecFile, errs: &mut SpecErrors) {
            ...
            if let Some(ref inline_services) = wl.services {
                for (sid, svc) in inline_services {
                    if let Some(ref ip) = svc.ip {
                        check_ip(ip, wl_path.key("services").key(sid).key("ip"), errs);
                    }
                }
            }
        }
    ...
    for (ip, locations) in &explicit_ips {
        if locations.len() > 1 {
            errs.error(
                locations[1].clone(),
                format!("duplicate IP '{}' (also assigned at {})", ip, locations[0]),
            );
        }
    }
}
...
///
/// IPs are NOT allocated here — the orchestrator handles allocation.
/// Explicit IPs are passed through; auto-assigned resources get empty IP/MAC strings.
fn build_namespace_spec(
    ...
    if let Some(ref spec_workloads) = spec.workloads {
        for (wid, wl) in spec_workloads {
            ...
            // Pass explicit IP through, or empty string for auto-assign.
            let pod_ip = wl.ip.clone().unwrap_or_default();
            ...
            workloads.insert(
                ...
                WorkloadSpec {
                    network: Some(PodNetworkConfig {
                        ip: pod_ip,
                        mac: String::new(), // orchestrator generates MAC
                    }),
                    containers,
                    suspend_on_idle,
                    resources,
            ...
            if let Some(ref inline_services) = wl.services {
                for (sid, svc) in inline_services {
                    let svc_ip = svc.ip.clone().unwrap_or_default();
                    ...
                    let idle_timeout_ms = svc
                        ...
                        .map(|s| parse_duration_ms(s).expect("validated earlier"))
                        .unwrap_or(0);
                    let (buffer_frames, buffer_timeout_ms) = convert_buffer(&svc.buffer);
