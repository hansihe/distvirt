...
use crate::sm::{
    ...
    EndpointInfo, WireGuardPeerEndpointInfo, WireGuardPeerId,
};
...
pub(crate) struct EndpointAdapter {
    ...
    endpoint_cache: HashMap<EndpointId, EndpointInfo>,
...
impl EndpointAdapter {
    ...
    pub(crate) fn reconcile(&mut self, router: &mut DRouter) -> (Vec<EndpointAction>, bool) {
        ...
        for action in &actions {
            match action {
                EndpointAction::Update { endpoint_id, info } => {
                    self.endpoint_cache.insert(*endpoint_id, info.clone());
                }
                EndpointAction::Remove { endpoint_id, .. } => {
                    self.endpoint_cache.remove(endpoint_id);
                }
    ...
    pub(crate) fn build_endpoint_sync(&self) -> Vec<(EndpointId, EndpointInfo)> {
        self.endpoint_cache
            .iter()
    ...
    fn added(&self, input: &EndpointsInputSource) -> Option<Vec<EndpointAction>> {
        match input {
            EndpointsInputSource::EndpointEndpointInfo(endpoint_id, Some(info)) => {
                Some(vec![EndpointAction::Update {
    ...
    fn removed(&self, input: &EndpointsInputSource) -> Option<Vec<EndpointAction>> {
        match input {
            EndpointsInputSource::EndpointEndpointInfo(endpoint_id, Some(old)) => {
                Some(vec![EndpointAction::Remove {
                    endpoint_id: *endpoint_id,
                    old_info: old.clone(),
                }])
            }
    ...
    fn changed(
        ...
        match (old, new) {
            (
                ...
                (_, Some(info)) => Some(vec![EndpointAction::Update {
                    endpoint_id: *endpoint_id,
                    info: info.clone(),
                }]),
